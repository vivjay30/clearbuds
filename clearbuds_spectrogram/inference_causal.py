...
def log_mel_spec(y, sample_rate):
    ...
    S = librosa.feature.melspectrogram(S=power_spectrogram, sr=sample_rate, n_mels=n_mels, fmin=fmin, fmax=fmax)
...
def infer(model, device, args):
    ...
    # Zero pad the front so that the first couple of frames have the context needed
    ...
    sf.write(os.path.join(args.output_dir, "output.wav"), output * 2, args.sample_rate)
...
def main(args):
    ...
    state_dict = torch.load(args.checkpoints_path, map_location=device).state_dict()
