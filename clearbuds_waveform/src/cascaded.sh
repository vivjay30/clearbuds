...
CUDA_VISIBLE_DEVICES=0 python evaluate_recorded.py --model-path checkpoints/clearvoice_iphone_causal_anechoic_reverb/final.pth.tar \
											        --n-mics 2 --sample-rate 15625 --file-path-left $FILE_L --use-cuda=1;
...
CUDA_VISIBLE_DEVICES=0 python inference.py inputs checkpoints/model.pt outputs  --use-cuda --sample-rate 15625 --chunk-size 25600 --cutoff .003;
mv outputs/output.wav ../real_output.wav;

cd ../clearbuds_waveform;

# For doing spectrogram only
# CUDA_VISIBLE_DEVICES=2 python inference_causal.py inputs checkpoints/model.pt outputs  --use-cuda --sample-rate 15625 --chunk-size 25600 --cutoff .006 --spectrogram-only;
